...
const BASE_URL = "https://wishpeti.com";
const emailLayout = (title, body, buttonText, buttonUrl, imageUrl)=>`
  <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
    ...
    <p style="margin-top: 30px; font-size: 12px; color: #94a3b8; border-top: 1px solid #e2e8f0; padding-top: 20px;">
      © 2026 Wishpeti. Keeping your connections private and secure.
    </p>
  </div>
`;
serve(async (req)=>{
  try {
    ...
    const { data: profile } = await supabase.from('creator_profiles').select('email, display_name').eq('id', record.creator_id).single();
    // 2. Fetch Item Details from wishlist_items
    let itemName = "a gift from your wishlist";
    let itemImage = null;
    if (record.item_id) {
      const { data: itemData, error: itemError } = await supabase.from('wishlist_items').select('title, image_url').eq('id', record.item_id).single();
    ...
    if (record.payment_status === 'paid' && old_record?.payment_status !== 'paid') {
      if (senderEmail) {
        notifications.push(sendEmail({
          to: senderEmail,
          subject: `Thanks! Your gift for ${creatorName} is confirmed`,
          html: emailLayout("Gift Confirmed! 🎁", `Hi ${record.buyer_name || 'there'}, <br><br>your payment for <strong>${itemName}</strong> was successful.`, "Track Your Gift", trackingPage, itemImage)
        }));
      }
      if (creatorEmail) {
        notifications.push(sendEmail({
          to: creatorEmail,
          subject: `Good news! Someone just sent you a gift`,
          html: emailLayout("You've got a gift! 🎈", `Hi ${creatorName}, <br><br>someone just fulfilled <strong>${itemName}</strong>.`, "Go to Manage Gifts", `${BASE_URL}/manage-gifts`, itemImage)
        }));
      }
    }
    // SCENARIO: ORDER ACCEPTED (To Sender)
...
async function sendEmail({ to, subject, html }) {
  return fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'Wishpeti <hello@wishpeti.com>',
